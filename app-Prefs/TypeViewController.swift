...
class TypeViewController: UIViewController {
    ...
    func doneItemDidClicked(_ sender: AnyObject) {
        ...
        var msg = ""
        if cateCell?.detailTextLabel?.text == "click to selected" {
...
extension TypeViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0 {
            ...
            cell.titleLabel.text = indexPath.row == 1 ? "title" : "action"
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        let label = UILabel(frame: CGRect(x: 25, y: 10, width: UIScreen.main.bounds.width - 50, height: 80))
        label.font = UIFont.systemFont(ofSize: 10)
        label.textAlignment = .center
        label.textColor = UIColor.darkGray
        label.numberOfLines = 0
        label.text = "系统设置动作请直接输入地址，如 无线局域网：root=WIFI;\n应用动作轻输入完整链接，如 微博扫二维码：sinaweibo://qrcode;\n应用动作需要自己搜索，如果多个应用的 URL Scheme 相同，则系统默认选择最后安装的打开;\n添加到首页后请点击测试是否正常打开。"
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.row == 0 {
            let cateSheet = UIAlertController(title: "select your action category", message: "", preferredStyle: .actionSheet)
            let cancel = UIAlertAction(title: "Cancel", style: .cancel, handler: { (_) in
                
            })
            let system = UIAlertAction(title: "Syetem Action", style: .default, handler: { (_) in
                let cell = tableView.cellForRow(at: IndexPath(row: 0, section: 0))
                cell?.detailTextLabel?.text = "System Action"
            })
            let custom = UIAlertAction(title: "Custom Action", style: .default, handler: { (_) in
                let cell = tableView.cellForRow(at: IndexPath(row: 0, section: 0))
                cell?.detailTextLabel?.text = "Custom Action"
            })
